const { USER_NOT_FOUND, ROLE_ERROR, TOKEN_ERROR } = require("../constans");
...
const verifyToken = async (req, res, next) => {
  try {
    ...
    if (token) {
      ...
      const decoded = jwt.verify(token, process.env.JWT_SECRET_KIWI);
...
const isAdmin = async (req, res, next) => {
  try {
    ...
    if (user.role !== "admin")
      return res.status(401).json({ message: ROLE_ERROR });
    next();
  } catch (error) {
